...
```javascript
    ...
    // var [str1, str2, str3] = data;
    // console.log(str1, str2, str3);  // A1 A2 A3


    // 解包，顺序接收
    // var data = ["A1", "A2", "A3", "A4"];
    // var [str1, , str3] = data;
    // console.log(str1, str3); // A1 A3

    // // 解包，指定默认值，如果成员值为undefined则采用默认值，其他的都按位赋值，包括null
    // var data = ["A1", null, undefined, "A4"];
    // var [str1, str2 = "abc", str3 = "default"] = data;
    // console.log(str1, str2, str3); // A1 null default
...
```javascript
    // // 解包对象，按属性名赋值，不存在的属性，则为undefined
    // var xiaoming = { name: "xiaoming", age: 18, sex: true, address: "北京" };
    // var { name, address, age, mobile="1331234567", city} = xiaoming;
    // console.log(name, age, address, mobile, city); // xiaoming 18 北京 1331234567 undefined

    // 解包对象，属性重命名，并指定默认值
    var xiaoming = { name: "xiaoming", age: 18, sex: true, address: "北京" };
    var { name: myName, age: myage, mobile: mymobile} = xiaoming;
...
fn4(10,20,30,40,50); // 10 20 [30, 40, 50]


function fn5(x, y, ...surplus){
    console.log(x, y, surplus)
}
arr = [10,20,30,40,50]
fn5(...arr); // 10 20 [30, 40, 50]
```
...
| console.count(message) | 写在函数或方法内部，用于统计当前函数被调用的次数 |
...
BOM（Brower Object Model，浏览器对象模型），主要是浏览器中js解释器提供给开发者调用浏览器功能的对象或接口方法，因为旧版本浏览器厂商实现不一样，所以有可能存在同一段代码在不同浏览器下的结果不一致的兼容问题。当然现代浏览器几乎不存在兼容问题了。
